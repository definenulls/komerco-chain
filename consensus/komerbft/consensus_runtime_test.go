...
func TestConsensusRuntime_isFixedSizeOfEpochMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.KomerBFTConfig.EpochSize = c.epochSize
...
func TestConsensusRuntime_isFixedSizeOfSprintMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.KomerBFTConfig.SprintSize = c.sprintSize
		runtime.epoch.FirstBlockInEpoch = c.firstBlockInEpoch
		assert.False(t,
...
func TestConsensusRuntime_OnBlockInserted_EndOfEpoch(t *testing.T) {
	...
	komerbftBackendMock := new(komerbftBackendMock)
	komerbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validatorSet).Times(3)
	...
	config := &runtimeConfig{
		...
		komerbftBackend: komerbftBackendMock,
		txPool:         txPool,
...
func TestConsensusRuntime_OnBlockInserted_MiddleOfEpoch(t *testing.T) {
	...
	komerbftBackendMock := new(komerbftBackendMock)
	komerbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(nil).Once()
	...
	config := &runtimeConfig{
		KomerBFTConfig: &KomerBFTConfig{EpochSize: epochSize},
		blockchain:    blockchainMock,
		txPool:        txPool,
	}
...
func Test_NewConsensusRuntime(t *testing.T) {
	...
	komerbftBackendMock := new(komerbftBackendMock)
	komerbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators).Twice()
	...
	assert.Equal(t, uint64(10), runtime.config.KomerBFTConfig.SprintSize)
	assert.Equal(t, uint64(10), runtime.config.KomerBFTConfig.EpochSize)
	assert.Equal(t, "0x0000000000000000000000000000000000000101", contracts.ValidatorSetContract.String())
	assert.Equal(t, "0x1300000000000000000000000000000000000000", runtime.config.KomerBFTConfig.Bridge.BridgeAddr.String())
	assert.Equal(t, "0x1000000000000000000000000000000000000000", runtime.config.KomerBFTConfig.Bridge.CheckpointAddr.String())
	assert.True(t, runtime.IsBridgeEnabled())
	systemStateMock.AssertExpectations(t)
	blockchainMock.AssertExpectations(t)
	komerbftBackendMock.AssertExpectations(t)
}
...
func TestConsensusRuntime_calculateCommitEpochInput_SecondEpoch(t *testing.T) {
	...
	komerbftBackendMock := new(komerbftBackendMock)
	komerbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.getPublicIdentities()).Twice()
...
func TestConsensusRuntime_HasQuorum(t *testing.T) {
	...
	config := &runtimeConfig{
		...
		blockchain:    blockchainMock,
		KomerBFTConfig: &KomerBFTConfig{EpochSize: 10, SprintSize: 5},
	}
	runtime := &consensusRuntime{
		state:          state,
		config:         config,
