...
// komerbftBackend is an interface defining komerbft methods needed by fsm and sync tracker
type komerbftBackend interface {
...
func Factory(params *consensus.Params) (consensus.Consensus, error) {
	logger := params.Logger.Named("komerbft")
	...
	// initialize komerbft consensus config
	customConfigJSON, err := json.Marshal(params.Config.Config)
	if err != nil {
		return nil, err
	}

	err = json.Unmarshal(customConfigJSON, &komerbft.consensusConfig)
	if err != nil {
		return nil, err
	}
...
type Komerbft struct {
	...
	// consensusConfig is genesis configuration for komerbft consensus protocol
...
func GenesisPostHookFactory(config *chain.Chain, engineName string) func(txn *state.Transition) error {
	return func(transition *state.Transition) error {
		komerBFTConfig, err := GetKomerBFTConfig(config)
		if err != nil {
			return err
		}

		// initialize ChildValidatorSet SC
		input, err := getInitChildValidatorSetInput(komerBFTConfig)
		if err != nil {
			return err
		}
		...
		input, err = getInitChildERC20PredicateInput(komerBFTConfig.Bridge)
		if err != nil {
			return err
		}
		...
		if komerBFTConfig.Bridge != nil {
			rootNativeERC20Token = komerBFTConfig.Bridge.RootNativeERC20Addr
		}

		if komerBFTConfig.MintableERC20Token {
			...
			params := &contractsapi.InitializeNativeERC20MintableFn{
				...
				Owner_:     komerBFTConfig.Governance,
...
func (p *Komerbft) Initialize() error {
	p.logger.Info("initializing komerbft...")
	...
	// initialize komerbft consensus data directory
	p.dataDir = filepath.Join(p.config.Config.Path, "komerbft")
...
func (p *Komerbft) Start() error {
	p.logger.Info("starting komerbft consensus", "signer", p.key.String())
...
func (p *Komerbft) initRuntime() error {
	runtimeConfig := &runtimeConfig{
		KomerBFTConfig:         p.consensusConfig,
		Key:                   p.key,
		DataDir:               p.dataDir,
		State:                 p.state,
...
func (p *Komerbft) startRuntime() error {
	go p.startConsensusProtocol()

	return nil
}

func (p *Komerbft) startConsensusProtocol() {
...
func (p *Komerbft) waitForNPeers() bool {
	for {
		select {
		case <-p.closeCh:
			return false
...
func (p *Komerbft) verifyHeaderImpl(parent, header *types.Header, parents []*types.Header) error {
