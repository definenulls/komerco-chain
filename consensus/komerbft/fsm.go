...
type fsm struct {
	// KomerBFT consensus protocol configuration
	...
	// komerbftBackend implements methods needed from the komerbft
...
func (f *fsm) Validate(proposal []byte) error {
	...
	if err := extra.ValidateParentSignatures(block.Number(), f.komerbftBackend, nil, f.parent, parentExtra,
	...
	if f.logger.IsTrace() && block.Number() > 1 {
		validators, err := f.komerbftBackend.GetValidators(block.Number()-2, nil)
		if err != nil {
...
func validateHeaderFields(parent *types.Header, header *types.Header) error {
	...
	// verify mix digest
	if header.MixHash != KomerBFTMixDigest {
