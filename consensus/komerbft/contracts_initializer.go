...
var (
	nativeTokenName     = "KOMERCO"
	nativeTokenSymbol   = "KMC"
...
func getInitChildValidatorSetInput(komerBFTConfig KomerBFTConfig) ([]byte, error) {
	apiValidators := make([]*contractsapi.ValidatorInit, len(komerBFTConfig.InitialValidatorSet))

	for i, validator := range komerBFTConfig.InitialValidatorSet {
	...
	params := &contractsapi.InitializeChildValidatorSetFn{
		Init: &contractsapi.InitStruct{
			EpochReward:   new(big.Int).SetUint64(komerBFTConfig.EpochReward),
			MinStake:      big.NewInt(minStake),
			MinDelegation: big.NewInt(minDelegation),
			EpochSize:     new(big.Int).SetUint64(komerBFTConfig.EpochSize),
		},
		NewBls:     contracts.BLSContract,
		Governance: komerBFTConfig.Governance,
