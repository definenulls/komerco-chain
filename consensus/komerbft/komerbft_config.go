...
func GetKomerBFTConfig(chainConfig *chain.Chain) (KomerBFTConfig, error) {
	consensusConfigJSON, err := json.Marshal(chainConfig.Params.Engine["komerbft"])
	if err != nil {
		return KomerBFTConfig{}, err
	}

	var komerBFTConfig KomerBFTConfig
	err = json.Unmarshal(consensusConfigJSON, &komerBFTConfig)

	if err != nil {
		return KomerBFTConfig{}, err
	}

	return komerBFTConfig, nil
}
