...
type runtimeConfig struct {
	...
	komerbftBackend        komerbftBackend
...
func (c *consensusRuntime) initStateSyncManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		stateSenderAddr := c.config.KomerBFTConfig.Bridge.BridgeAddr
		stateSyncManager, err := newStateSyncManager(
			...
			&stateSyncConfig{
				...
				stateSenderStartBlock: c.config.KomerBFTConfig.Bridge.EventTrackerStartBlocks[stateSenderAddr],
				jsonrpcAddr:           c.config.KomerBFTConfig.Bridge.JSONRPCEndpoint,
...
func (c *consensusRuntime) initCheckpointManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		...
		txRelayer, err := txrelayer.NewTxRelayer(txrelayer.WithIPAddress(c.config.KomerBFTConfig.Bridge.JSONRPCEndpoint))
		if err != nil {
			return err
		}
		...
		c.checkpointManager = newCheckpointManager(
			...
			c.config.KomerBFTConfig.Bridge.CheckpointAddr,
			txRelayer,
			c.config.blockchain,
			c.config.komerbftBackend,
...
func (c *consensusRuntime) IsBridgeEnabled() bool {
	return c.config.KomerBFTConfig.IsBridgeEnabled()
}
...
func (c *consensusRuntime) FSM() error {
	...
	blockBuilder, err := c.config.blockchain.NewBlockBuilder(
		...
		c.config.txPool,
		c.config.KomerBFTConfig.BlockTime,
		c.logger,
	)
	...
	ff := &fsm{
		config:            c.config.KomerBFTConfig,
		parent:            parent,
		backend:           c.config.blockchain,
		komerbftBackend:    c.config.komerbftBackend,
...
func (c *consensusRuntime) restartEpoch(header *types.Header) (*epochMetadata, error) {
	...
	validatorSet, err := c.config.komerbftBackend.GetValidators(header.Number, nil)
	if err != nil {
...
func (c *consensusRuntime) calculateCommitEpochInput(
	...
	if blockHeader.Number > commitEpochLookbackSize {
		for i := 0; i < commitEpochLookbackSize; i++ {
			validators, err := c.config.komerbftBackend.GetValidators(blockHeader.Number-2, nil)
			if err != nil {
				return nil, err
			}
...
func (c *consensusRuntime) isFixedSizeOfEpochMet(blockNumber uint64, epoch *epochMetadata) bool {
	return epoch.FirstBlockInEpoch+c.config.KomerBFTConfig.EpochSize-1 == blockNumber
}
...
func (c *consensusRuntime) isFixedSizeOfSprintMet(blockNumber uint64, epoch *epochMetadata) bool {
	return (blockNumber-epoch.FirstBlockInEpoch+1)%c.config.KomerBFTConfig.SprintSize == 0
}
