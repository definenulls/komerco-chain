...
import (
	...
	gensc "github.com/definenulls/komerco-chain/consensus/komerbft/contractsapi"
...
`
	...
	if err := ioutil.WriteFile("./consensus/komerbft/contractsapi/contractsapi.go", output, 0600); err != nil {
		log.Fatal(err)
	}
}
...
func generateType(generatedData *generatedData, name string, obj *abi.Type, res *[]string) (string, error) {
	...
	for _, tupleElem := range obj.TupleElems() {
		...
		// uint8 internally in komerbft, we can use regexp to replace those values with the
