...
import (
	...
	secretsHelper "github.com/definenulls/komerco-chain/secrets/helper"
...
func TestIntegration_CommitEpoch(t *testing.T) {
	...
	for _, currentValidators := range validatorSets {
		...
		// validator data for komerbft config
		...
		for i, validator := range accSet {
			...
			// create validator data for komerbft config
		...
		initInput, err := getInitChildValidatorSetInput(komerBFTConfig)
		require.NoError(t, err)
		...
		commitEpoch := createTestCommitEpochInput(t, 1, accSet, komerBFTConfig.EpochSize)
		...
		commitEpoch = createTestCommitEpochInput(t, 2, accSet, komerBFTConfig.EpochSize)
		input, err = commitEpoch.EncodeAbi()
		require.NoError(t, err)
