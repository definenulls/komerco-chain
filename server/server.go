...
import (
	...
	"github.com/definenulls/komerco-chain/blockchain/storage"
	"github.com/definenulls/komerco-chain/blockchain/storage/leveldb"
	"github.com/definenulls/komerco-chain/blockchain/storage/memory"
	consensusKomerBFT "github.com/definenulls/komerco-chain/consensus/komerbft"

	"github.com/definenulls/komerco-chain/archive"
	...
	"github.com/definenulls/komerco-chain/consensus/komerbft/statesyncrelayer"
	...
	configHelper "github.com/definenulls/komerco-chain/helper/config"
	"github.com/definenulls/komerco-chain/helper/progress"
	"github.com/definenulls/komerco-chain/jsonrpc"
	"github.com/definenulls/komerco-chain/network"
	"github.com/definenulls/komerco-chain/secrets"
	"github.com/definenulls/komerco-chain/server/proto"
	"github.com/definenulls/komerco-chain/state"
	itrie "github.com/definenulls/komerco-chain/state/immutable-trie"
	"github.com/definenulls/komerco-chain/state/runtime"
	"github.com/definenulls/komerco-chain/state/runtime/allowlist"
	"github.com/definenulls/komerco-chain/state/runtime/tracer"
	"github.com/definenulls/komerco-chain/txpool"
	"github.com/definenulls/komerco-chain/types"
	"github.com/definenulls/komerco-chain/validate"
...
type Server struct {
	...
	// stateSyncRelayer is handling state syncs execution (Komerbft exclusive)
...
func NewServer(config *Config) (*Server, error) {
	...
	if ConsensusType(engineName) == KomerBFTConsensus {
		komerBFTConfig, err := consensusKomerBFT.GetKomerBFTConfig(config.Chain)
		if err != nil {
			return nil, err
		}

		if komerBFTConfig.InitialTrieRoot != types.ZeroHash {
			checkedInitialTrieRoot, err := itrie.HashChecker(komerBFTConfig.InitialTrieRoot.Bytes(), stateStorage)
			if err != nil {
				return nil, fmt.Errorf("error on state root verification %w", err)
			}

			if checkedInitialTrieRoot != komerBFTConfig.InitialTrieRoot {
				return nil, errors.New("invalid initial state root")
			}

			logger.Info("Initial state root checked and correct")

			initialStateRoot = komerBFTConfig.InitialTrieRoot
		}
	}
...
func (s *Server) setupRelayer() error {
	...
	komerBFTConfig, err := consensusKomerBFT.GetKomerBFTConfig(s.config.Chain)
	if err != nil {
		return fmt.Errorf("failed to extract komerbft config: %w", err)
	}

	trackerStartBlockConfig := map[types.Address]uint64{}
	if komerBFTConfig.Bridge != nil {
		trackerStartBlockConfig = komerBFTConfig.Bridge.EventTrackerStartBlocks
	}
