...
func TestE2E_Consensus_RegisterValidator(t *testing.T) {
	...
	systemState := komerbft.NewSystemState(
	...
	extra, err := komerbft.GetIbftExtra(genesisBlock.ExtraData)
	require.NoError(t, err)
	...
	validators := komerbft.AccountSet{}
...
func TestE2E_Consensus_Validator_Unstake(t *testing.T) {
	...
	rootchainValidators := []*komerbft.ValidatorInfo{}
