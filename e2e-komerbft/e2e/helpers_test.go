...
func getRootchainValidators(relayer txrelayer.TxRelayer, checkpointManagerAddr ethgo.Address) ([]*komerbft.ValidatorInfo, error) {
	...
	validators := make([]*komerbft.ValidatorInfo, validatorsCount)
	...
	for i := 0; i < int(validatorsCount); i++ {
		...
		validators[i] = &komerbft.ValidatorInfo{
...
func sendExitTransaction(
	...
	proofExitEventEncoded, err := exitEventAPI.Encode(&komerbft.ExitEvent{
		ID:       exitEventID,
