...
import (
	...
	rootchainHelper "github.com/definenulls/komerco-chain/command/rootchain/helper"
	"github.com/definenulls/komerco-chain/command/sidechain"
	...
	"github.com/definenulls/komerco-chain/e2e-komerbft/framework"
...
func TestE2E_Bridge_DepositAndWithdrawERC20(t *testing.T) {
	...
	manifest, err := komerbft.LoadManifest(path.Join(cluster.Config.TmpDir, manifestFileName))
	require.NoError(t, err)
	...
	currentExtra, err := komerbft.GetIbftExtra(currentBlock.ExtraData)
	require.NoError(t, err)
...
func TestE2E_Bridge_ChangeVotingPower(t *testing.T) {
	...
	queryValidators := func(handler func(idx int, validatorInfo *komerbft.ValidatorInfo)) {
	...
	originalValidatorStorage := make(map[ethgo.Address]*komerbft.ValidatorInfo, votingPowerChanges)

	queryValidators(func(idx int, validator *komerbft.ValidatorInfo) {
