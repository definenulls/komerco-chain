...
import (
	...
	txpoolProto "github.com/definenulls/komerco-chain/txpool/proto"
...
func (t *TestServer) Start() {
	...
	args := []string{
		"server",
		// add data dir
		"--" + komerbftsecrets.AccountDirFlag, config.DataDir,
		// add custom chain
		"--chain", config.Chain,
		// enable p2p port
...
func (t *TestServer) Stake(amount uint64) error {
	args := []string{
		"komerbft",
		"stake",
		"--" + komerbftsecrets.AccountDirFlag, t.config.DataDir,
...
func (t *TestServer) RegisterValidator(secrets string, stake string) error {
	args := []string{
		"komerbft",
		"register-validator",
		"--" + komerbftsecrets.AccountDirFlag, path.Join(t.clusterConfig.TmpDir, secrets),
		"--jsonrpc", t.JSONRPCAddr(),
	}
...
func (t *TestServer) Delegate(amount uint64, secrets string, validatorAddr ethgo.Address) error {
	args := []string{
		"komerbft",
		"stake",
		"--" + komerbftsecrets.AccountDirFlag, secrets,
		"--jsonrpc", t.JSONRPCAddr(),
...
func (t *TestServer) HasValidatorSealed(firstBlock, lastBlock uint64, validators komerbft.AccountSet,
	...
	for i := firstBlock + 1; i <= lastBlock; i++ {
		...
		extra, err := komerbft.GetIbftExtra(block.ExtraData)
		if err != nil {
			return false, err
		}
