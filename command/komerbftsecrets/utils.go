...
// common flags for all komerbft commands
const (
	AccountDirFlag    = "data-dir"
	AccountConfigFlag = "config"

	AccountDirFlagDesc    = "the directory for the Komerco Edge data if the local FS is used"
	AccountConfigFlagDesc = "the path to the SecretsManager config file, if omitted, the local FS secrets manager is used"
)

// common errors for all komerbft commands
var (
