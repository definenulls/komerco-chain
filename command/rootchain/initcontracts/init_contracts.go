...
import (
	...
	"github.com/definenulls/komerco-chain/consensus/komerbft/contractsapi/artifact"
...
var (
	...
	metadataPopulatorMap = map[string]func(*komerbft.RootchainConfig, types.Address){
		stateSenderName: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
			rootchainConfig.StateSenderAddress = addr
		},
		checkpointManagerName: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
			rootchainConfig.CheckpointManagerAddress = addr
		},
		blsName: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
			rootchainConfig.BLSAddress = addr
		},
		bn256G2Name: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
			rootchainConfig.BN256G2Address = addr
		},
		exitHelperName: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
			rootchainConfig.ExitHelperAddress = addr
		},
		rootERC20PredicateName: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootERC20PredicateAddress = addr
		},
		rootERC20Name: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootNativeERC20Address = addr
		},
		erc20TemplateName: func(rootchainConfig *komerbft.RootchainConfig, addr types.Address) {
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	manifest, err := komerbft.LoadManifest(params.manifestPath)
	if err != nil {
...
func deployContracts(outputter command.OutputFormatter, client *jsonrpc.Client,
	manifest *komerbft.Manifest) error {
	...
	rootchainConfig := &komerbft.RootchainConfig{}
...
func initializeExitHelper(txRelayer txrelayer.TxRelayer, rootchainConfig *komerbft.RootchainConfig,
	deployerKey ethgo.Key) error {
...
func initializeRootERC20Predicate(txRelayer txrelayer.TxRelayer, rootchainConfig *komerbft.RootchainConfig,
	deployerKey ethgo.Key) error {
...
func validatorSetToABISlice(o command.OutputFormatter,
	validators []*komerbft.Validator) ([]*contractsapi.Validator, error) {
	accSet := make(komerbft.AccountSet, len(validators))
	...
	for i, validator := range validators {
		...
		accSet[i] = &komerbft.ValidatorMetadata{
