...
func (p *genesisParams) validateFlags() error {
	...
	if p.epochSize < 2 && (p.isIBFTConsensus() || p.isKomerBFTConsensus()) {
...
func (p *genesisParams) isKomerBFTConsensus() bool {
	return server.ConsensusType(p.consensusRaw) == server.KomerBFTConsensus
}
...
func (p *genesisParams) initRawParams() error {
	...
	if p.consensus == server.KomerBFTConsensus {
		return nil
	}
