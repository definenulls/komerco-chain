...
func setFlags(cmd *cobra.Command) {
	cmd.Flags().StringVar(
		...
		"the directory for the Komerco Edge genesis data",
	)
...
func setLegacyFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Uint64Var(
		...
		chainIDFlagLEGACY,
		command.DefaultChainID,
		"the ID of the chain (not-applicable for Komerbft consensus protocol as chain id is defined in manifest.json)",
	)
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	if params.isKomerBFTConsensus() {
		err = params.generateKomerBftChainConfig(outputter)
	} else {
