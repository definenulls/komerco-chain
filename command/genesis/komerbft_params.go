...
// generateKomerBftChainConfig creates and persists komerbft chain configuration to the provided file path
func (p *genesisParams) generateKomerBftChainConfig(o command.OutputFormatter) error {
	// load manifest file
	manifest, err := komerbft.LoadManifest(p.manifestPath)
	if err != nil {
	...
	var bridge *komerbft.BridgeConfig
	...
	komerBftConfig := &komerbft.KomerBFTConfig{
	...
	chainConfig := &chain.Chain{
		...
		Params: &chain.Params{
			...
			Engine: map[string]interface{}{
				string(server.KomerBFTConsensus): komerBftConfig,
			},
		},
	...
	validatorMetadata := make([]*komerbft.ValidatorMetadata, len(manifest.GenesisValidators))
	...
	genesisExtraData, err := generateExtraDataKomerBft(validatorMetadata)
	if err != nil {
		return err
	}
	...
	chainConfig.Genesis = &chain.Genesis{
		...
		Mixhash:    komerbft.KomerBFTMixDigest,
	}
...
// generateExtraDataKomerBft populates Extra with specific fields required for komerbft consensus protocol
func generateExtraDataKomerBft(validators []*komerbft.ValidatorMetadata) ([]byte, error) {
	delta := &komerbft.ValidatorSetDelta{
		Added:   validators,
		Removed: bitmap.Bitmap{},
	}

	extra := komerbft.Extra{Validators: delta, Checkpoint: &komerbft.CheckpointData{}}

	return append(make([]byte, komerbft.ExtraVanity), extra.MarshalRLPTo(nil)...), nil
}
