...
func ValidateSecretFlags(dataDir, config string) error {
	if config == "" {
		if dataDir == "" {
			return komerbftsecrets.ErrInvalidParams
		} else {
...
func GetAccount(accountDir, accountConfig string) (*wallet.Account, error) {
	...
	secretsManager, err := komerbftsecrets.GetSecretsManager(accountDir, accountConfig, true)
	if err != nil {
		return nil, err
	}
